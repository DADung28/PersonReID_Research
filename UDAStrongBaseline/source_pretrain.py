...
def main_worker(args):
    ...
    dataset_source, num_classes, train_loader_source, test_loader_source = \
        get_data(args.traindata, args.data_dir, args.height,
                 args.width, args.batch_size, args.workers, args.num_instances, iters)
    dataset_source1, num_classes, train_loader_source1, test_loader_source1 = \
        get_data(args.dataset_source, args.test_dir, args.height,
                 args.width, args.batch_size, args.workers, args.num_instances, iters)
        
    dataset_target, _, train_loader_target, test_loader_target = \
        get_data(args.dataset_target, args.test_dir, args.height,
                 args.width, args.batch_size, args.workers, 0, iters)
    # Create model
    if args.arch == 'Swin':
        model = models.create(name = args.arch, mb_h = 1024, num_features=args.features, dropout=args.dropout, num_classes=[num_classes])
    elif args.arch == 'ViT':
        model = models.create(name = args.arch, mb_h = 768, num_features=args.features, dropout=args.dropout, num_classes=[num_classes])
    else:
    ...
    print('Evalute train dataset')
    (_, mAP_s) = evaluator.evaluate(test_loader_source, dataset_source.query,
                                dataset_source.gallery, cmc_flag=True, rerank=False, cam_overlap=True)
    # args.evaluate=True
    if args.evaluate:
        print("Test on source domain:")
        evaluator.evaluate(test_loader_source1, dataset_source1.query, dataset_source1.gallery, cmc_flag=True, rerank=args.rerank)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if ((epoch+1)%args.eval_step==0 or (epoch==args.epochs-1)):
            ...
            print('Evalute source dataset')
            (_, mAP_s) = evaluator.evaluate(test_loader_source1, dataset_source1.query,
                                        dataset_source1.gallery, cmc_flag=True, rerank=False)
            print('Evalute terget dataset')
            (_, mAP_t) = evaluator.evaluate(test_loader_target, dataset_target.query,
                                        dataset_target.gallery, cmc_flag=True, rerank=False)
            is_best = mAP_t > best_mAP
            best_mAP = max(mAP_t, best_mAP)
            save_checkpoint({
            ...
            print('\n * Finished epoch {:3d}  source mAP: {:5.1%}  target mAP: {:5.1%}  best: {:5.1%}{}\n'.
                  format(epoch, mAP_s, mAP_t, best_mAP, ' *' if is_best else ''))
...
if __name__ == '__main__':
    ...
    # data
    parser.add_argument('-train', '--traindata', type=str, default='market1501',
    ...
    parser.add_argument('--test-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, '/home/jun/ReID_Dataset/'))
    parser.add_argument('--data-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, '/home/jun/ReID_Dataset/'))
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'logs/market1501TOdukemtmc/market_pretrain_resnet'))
