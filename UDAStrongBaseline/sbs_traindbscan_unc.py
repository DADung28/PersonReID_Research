...
def main_worker(args):
    ...
    model_1_ema.module.classifier0_3500 = nn.Linear(mb_h, ncs[0]+source_classes, bias=False).cuda()
    ...
    for epoch in range(len(clusters)):
        ...
        # Optimizer
        params = []
        if epoch%10<args.schedule_step:
            flag = 1.0
        else:
            flag = 0.1
        ...
        print(f'Epoch {epoch}, lr: {get_lr(optimizer)}')
            #lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=args.schedule_step, gamma=0.1)
        ...
        # Trainer
        if args.arch in ['ResNet50_multi', 'resnet50_multi', 'resnet50_multi_sbs']: 
            ...
            trainer = DbscanBaseTrainer_unc_ema_single(model_1, model_1_ema, contrast, None,None,
                                                num_cluster=ncs, c_name=ncs,alpha=args.alpha, fc_len=fc_len,
                                                source_classes=source_classes, uncer_mode=args.uncer_mode)
        ...
        if ((epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1)):
            ...
            (_, mAP_reranked_1), (_, mAP_1) = evaluator_1.evaluate(test_loader_target, dataset_target.query,
                                        dataset_target.gallery, cmc_flag=True, rerank=True)
            (_, mAP_reranked_2), (_, mAP_2) = evaluator_1_ema.evaluate(test_loader_target, dataset_target.query,
                                        dataset_target.gallery, cmc_flag=True, rerank=True)

            is_best = (mAP_1 > best_mAP) or (mAP_2 > best_mAP)
            best_mAP = max(mAP_1, mAP_2, best_mAP)

            save_model(model_1, (is_best) and (mAP_1 >= mAP_2), best_mAP, 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-instances', type=int, default=4,
                        help="each minibatch consist of "
                             ...
                             "default: 0 (NOT USE)")
    # model
    parser.add_argument('-a', '--arch', type=str, default='hrnet',
    ...
    parser.add_argument('--init-1', type=str,
                        default='logs/market1501TOdukemtmc/market_pretrain_hrnet/model_best.pth.tar',
                        metavar='PATH')
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default='logs/dbscan-market1501TOdukemtmc/hrnet')
