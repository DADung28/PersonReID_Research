...
def main_worker(args):
    ...
    dataset_source1, _, train_loader_source1, test_loader_source1 = \
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if ((epoch+1)%args.eval_step==0 or (epoch==args.epochs-1)):
            print('Evalute train dataset')
            (_, mAP_train) = evaluator.evaluate(test_loader_source, dataset_source.query,
                                        dataset_source.gallery, cmc_flag=True, rerank=False, cam_overlap=True)
            ...
            is_best = mAP_train > best_mAP
            best_mAP = max(mAP_train, best_mAP)
            save_checkpoint({
            ...
            print('\n * Finished epoch {:3d}  train mAP: {:5.1%} source mAP: {:5.1%}  target mAP: {:5.1%}  best: {:5.1%}{}\n'.
                  format(epoch, mAP_train, mAP_s, mAP_t, best_mAP, ' *' if is_best else ''))
