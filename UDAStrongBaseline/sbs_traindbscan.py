...
from UDAsbs.trainers import DbscanBaseTrainer, DbscanBaseTrainer_target, DbscanBaseTrainer_both, DbscanBaseTrainer_both_triplet, DbscanBaseTrainer_target_contrast, DbscanBaseTrainer_both_contrast
...
start_epoch = best_mAP = best_mAP_rerank = 0
...
def main_worker(args):
    global start_epoch, best_mAP, best_mAP_rerank
    ...
    # Get training dataset from target dataset 
    dataset_target, label_dict = get_data(args.dataset_target, args.data_dir, len(ncs))
    
    # Loading target dataset into dataloader with test transforms (dataset_target.query and dataset.gallery(path, person ids, camera ids))
    test_loader_target = get_test_loader(dataset_target, args.height, args.width, args.batch_size, args.workers)
    # Loading target dataset into dataloader with test transforms (dataset.train)
    ...
    # Get training dataset from source dataset  
    dataset_source, _ = get_data(args.dataset_source, args.data_dir, len(ncs))
    
    # Loading source dataset into dataloader with test transform (dataset.train)
    ...
    # Loading source dataset into dataloader with train transform (dataset.train)
    ...
    # img, horizontal_flipped_img, path, person_id, camera_id, image_id = next(iter(train_loader_source.loader))
    
    source_classes = dataset_source.num_train_pids # source_classes = 751 person ids in train dataset

    # Get number of images per ids sorted
    ...
    # Create 2 model
    model_1, _ = create_model(args, mb_h, [fc_len for _ in range(len(ncs))])
    # print(model_1)


    epoch = 0
    target_features_dict, _ = extract_features(model_1, tar_cluster_loader, print_freq=100)
    ...
    # Encode features by using jaccard distance
    ...
    model_1.module.classifier0_3500 = nn.Linear(mb_h, ncs[0]+source_classes, bias=False).cuda()
    ...
    print('==>Initiate mAP:')
    (_, mAP_1) = evaluator_1.evaluate(test_loader_target, dataset_target.query,
                                        dataset_target.gallery, cmc_flag=True, rerank=False)
    ...
    k_memory=8192
    contrast = onlinememory(mb_h, len(new_dataset),sour_numclass=source_classes,K=k_memory+source_classes,
    ...
    contrast.memory = torch.cat((source_centers, torch.rand(k_memory, mb_h)), dim=0).cuda()
    ...
    #print(evaluator_1.evaluate(test_loader_target, dataset_target.query,
    ...
    for epoch in range(len(clusters)):
        ...
        if epoch % args.dbscan_epoch == 0 and epoch !=0:
            target_features_dict, _ = extract_features(model_1, tar_cluster_loader, print_freq=50)
        ...
        train_loader_target = get_train_loader(dataset_target, args.height, args.width, cc,
                                               args.batch_size, args.workers, args.num_instances, iters, new_dataset)
        ...
        for key, value in model_1.named_parameters():
            if not value.requires_grad:
                print(key)
                continue
            params += [{"params": [value], "lr": args.lr*flag, "weight_decay": args.weight_decay, "momentum":args.momentum}]
        ...
        # Trainer
        if args.train_type == 'target':
            trainer = DbscanBaseTrainer_target(model_1, contrast,
                             num_cluster=ncs, alpha=args.alpha, fc_len=fc_len)
        elif args.train_type == 'source+target': 
            trainer = DbscanBaseTrainer_both(model_1, contrast,
                             num_cluster=ncs, alpha=args.alpha, fc_len=fc_len)
        elif args.train_type == 'source+target_tri': 
            trainer = DbscanBaseTrainer_both_triplet(model_1, contrast,
                             num_cluster=ncs, alpha=args.alpha, fc_len=fc_len)
        elif args.train_type == 'source+target': 
            trainer = DbscanBaseTrainer_target_contrast(model_1, contrast,
                             num_cluster=ncs, alpha=args.alpha, fc_len=fc_len)
        elif args.train_type == 'source+target': 
            trainer = DbscanBaseTrainer_both_contrast(model_1, contrast,
                             num_cluster=ncs, alpha=args.alpha, fc_len=fc_len)
        else:
            pass
        ...
        if ((epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1)):
            ...
            print('\n * Finished epoch {:3d}  model no.1 mAP: {:5.1%}  best: {:5.1%}\n'.
                  format(epoch, mAP_1, best_mAP, ' *' if is_best else ''))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_type', type=str, default='one_dataset', help = 'target, source+target, source+target_triplet, target_contrast,  source+target_contrast')
    ...
    parser.add_argument('--dbscan_epoch', type=int, default=6)
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default = '/home/jun/UDAStrongBaseline/logs/dbscan-market1501TOdukemtmc/resnet50_sbs_mb')
