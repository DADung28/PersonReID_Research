...
class DbscanBaseTrainer_unc_ema_single(object):
    ...
    def train(self, epoch, data_loader_target, data_loader_source, optimizer, choice_c, lambda_tri=1.0
        ...
        for i in range(train_iters):
            ...
            loss_ce_1, loss_reg, exp_variance = self.update_variance(items[2], p_out_t1, p_out_t1_ema, ml_sour, ml_sour_ema, f_out_t1, f_out_t1_ema)
    ...
    def update_variance(self, labels, pred1, pred1_ema, ml_sour, ml_sour_ema,f_out_t1,f_out_t1_ema):
        ...
        if only_sour:
            ...
            variance = torch.sum(self.kl_distance(self.log_sm(pred1), self.sm(pred1_ema.detach())), dim=1) \
            ...
            #variance = torch.sum(self.kl_distance(self.log_sm(torch.cat((pred1,ml_sour),1)), self.sm(torch.cat((pred1_ema,ml_sour_ema),1).detach())), dim=1)
        ...
        loss = torch.mean(loss_4layer * exp_variance) 
...
class DbscanBaseTrainer_new(object):
    def __init__(self, model_1, contrast, num_cluster=None, alpha=0.999, fc_len=3000):
        super().__init__()
        self.model_1 = model_1
    ...
    def train(self, epoch, data_loader_target, data_loader_source, optimizer, choice_c,
        ...
        for i in range(train_iters):
            ...
            inputs_1_t, inputs_2_t, pid_t, index_t = items[0], items[1], items[2], items[-1]
            inputs_1_s, inputs_2_s, pid_s, index_s = items_source[0], items_source[1], items_source[2], items_source[-1]
            ...
            f_out_s1, f_out_t1 = self.derange_spbn(f_out)
            p_out_s1, p_out_t1 = self.derange_spbn(p_out[0])
            memory_f_s1, memory_f_t1 = self.derange_spbn(memory_f)



            loss_tri_t = self.criterion_tri(f_out_t1, f_out_t1, pid_t)
            #loss_tri_s = self.criterion_tri(f_out_s1, f_out_s1, items[choice_c + 2])
            
            loss_ce_t = self.criterion_ce(p_out_t1, pid_t)
            #loss_ce_s = self.criterion_ce(p_out_s1, items_source[2])
            #contra_loss_instance, contra_loss_center, ml_sour, ml_sour_ema = \
            #self.contrast(memory_f_t1, f_out_s1, f_out_t1, f_out_t1_ema, index_t, items_source[2], epoch=epoch) 
            #torch.tensor(0),torch.tensor(0),torch.tensor(0),torch.tensor(0)


            #oss = loss_ce_1 + loss_tri_1
            loss = loss_ce_t + loss_tri_t
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            losses_ce[0].update(loss_ce_t.item())
            losses_tri[0].update(loss_tri_t.item())
            precisions[0].update(prec_1[0]) 
            # print log #
            batch_time.update(time.time() - end)
            end = time.time()
...
class DbscanBaseTrainer_target(object):
    def __init__(self, model_1, contrast, num_cluster=None, alpha=0.999, fc_len=3000):
        super().__init__()
        self.model_1 = model_1
...
class DbscanBaseTrainer_both(object):
    def __init__(self, model_1, contrast, num_cluster=None, alpha=0.999, fc_len=3000):
        super().__init__()
        self.model_1 = model_1
    ...
    def train(self, epoch, data_loader_target, data_loader_source, optimizer, choice_c,
        ...
        losses_ce_t = [AverageMeter(), AverageMeter()]
        losses_tri_t = [AverageMeter(), AverageMeter()]
        losses_ce_s = [AverageMeter(), AverageMeter()]
        losses_tri_s = [AverageMeter(), AverageMeter()]
        ...
        for i in range(train_iters):
            ...
            loss_tri_t = self.criterion_tri(f_out_t1, f_out_t1, pid_t)
            loss_tri_s = self.criterion_tri(f_out_s1, f_out_s1, pid_s)
            
            loss_ce_t = self.criterion_ce(p_out_t1, pid_t)
            loss_ce_s = self.criterion_ce(p_out_s1, pid_s)
            #contra_loss_instance, contra_loss_center, ml_sour, ml_sour_ema = \
            #self.contrast(memory_f_t1, f_out_s1, f_out_t1, f_out_t1_ema, index_t, items_source[2], epoch=epoch) 
            #torch.tensor(0),torch.tensor(0),torch.tensor(0),torch.tensor(0)
            ...
            #oss = loss_ce_1 + loss_tri_1
            loss = loss_ce_t + loss_tri_t + loss_ce_s + loss_tri_s
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            losses_ce_t[0].update(loss_ce_t.item())
            losses_tri_t[0].update(loss_tri_t.item())
            losses_ce_s[0].update(loss_ce_s.item())
            losses_tri_s[0].update(loss_tri_s.item())
            precisions[0].update(prec_1[0]) 
            # print log #
            batch_time.update(time.time() - end)
            end = time.time()
            ...
            if (i + 1) % print_freq == 1:
                print('Epoch: [{}][{}/{}]\t'
                      ...
                      .format(epoch, i, len(data_loader_target),
                              batch_time.val, batch_time.avg,
                              data_time.val, data_time.avg,
                              losses_ce_t[0].avg, losses_ce_s[0].avg,
                              losses_tri_t[0].avg, losses_tri_s[0].avg,
                              precisions[0].avg))
...
class DbscanBaseTrainer_both(object):
    ...
    def train(self, epoch, data_loader_target, data_loader_source, optimizer, choice_c,
        ...
        precisions_t = [AverageMeter(), AverageMeter()]
        precisions_s = [AverageMeter(), AverageMeter()]
        ...
        for i in range(train_iters):
            ...
            prec_t, = accuracy(p_out_t1.data, pid_t.data)
            prec_s, = accuracy(p_out_s1.data, pid_s.data)

            losses_ce_t[0].update(loss_ce_t.item())
            losses_tri_t[0].update(loss_tri_t.item())
            losses_ce_s[0].update(loss_ce_s.item())
            losses_tri_s[0].update(loss_tri_s.item())
            precisions_t[0].update(prec_t[0]) 
            precisions_s[0].update(prec_s[0]) 
            # print log #
            batch_time.update(time.time() - end)
            end = time.time()
            ...
            if (i + 1) % print_freq == 1:
                print('Epoch: [{}][{}/{}]\t'
                      ...
                      .format(epoch, i, len(data_loader_target),
                              ...
                              losses_ce_t[0].avg, losses_ce_s[0].avg,
                              losses_tri_t[0].avg, losses_tri_s[0].avg,
                              precisions_t[0].avg, precisions_s[0].avg))
...
class DbscanBaseTrainer_both_triplet(object):
    def __init__(self, model_1, contrast, num_cluster=None, alpha=0.999, fc_len=3000):
        super().__init__()
        self.model_1 = model_1
    ...
    def train(self, epoch, data_loader_target, data_loader_source, optimizer, choice_c,
        ...
        for i in range(train_iters):
            ...
            #oss = loss_ce_1 + loss_tri_1
            loss = loss_ce_t + loss_tri_t + loss_tri_s
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
class DbscanBaseTrainer_both_contrast(object):
    def __init__(self, model_1, contrast, num_cluster=None, alpha=0.999, fc_len=3000):
        super().__init__()
        self.model_1 = model_1
    ...
    def train(self, epoch, data_loader_target, data_loader_source, optimizer, choice_c,
        ...
        for i in range(train_iters):
            ...
            contra_loss_instance, contra_loss_center, ml_sour, ml_sour_ema = self.contrast(memory_f_t1, f_out_s1, f_out_t1, index_t, pid_s, epoch=epoch) 
...
class DbscanBaseTrainer_target_contrast(object):
    def __init__(self, model_1, contrast, num_cluster=None, alpha=0.999, fc_len=3000):
        super().__init__()
        self.model_1 = model_1
