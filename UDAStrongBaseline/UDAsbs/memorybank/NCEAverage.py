...
class onlinememory(nn.Module):
    ...
    def memo_circle_loss(self,index,q1):
        batchSize = q1.shape[0]
        # import ipdb;ipdb.set_trace()
        pseudo_label = torch.tensor([self.index2label[self.choice_c][i.item()] for i in index],
                                    dtype=torch.long).cuda()
    ...
    def forward(self, q1, q2, tar_tri, tar_tri_ema, index, sour_labels, epoch=0):
        # memory_f_t1, f_out_s1, f_out_t1, f_out_t1_ema, index_t, items_source[2], epoch=epoch
        batchSize = q1.shape[0]
        ...
        loss_q1 = self.memo_circle_loss(index + self.sour_numclass, q1)
