...
class ViT(nn.Module):
    ...
    def forward(self, x, feature_withbn=False, training=False):
        ...
        x = x.clamp(min=nn.Parameter(torch.ones(1) * 3)).pow(3)
        x = torch.nn.functional.adaptive_avg_pool2d(x, (1,768)).pow(1. / 3)
...
# Test the model, before you train it. 
    ...
    #net = timm.create_model(f'vit_base_patch16_224', pretrained=True, num_classes=751)
    ...
    # Sample data
    net = net.to('cpu')
    input = Variable(torch.FloatTensor(32, 3, 224, 224)).to('cpu')
    outputs = net(input)
