...
from .evaluation_metrics import cmc, cmc_nocam, mean_ap, mean_ap_nocam
...
def evaluate_all(query_features, gallery_features, distmat, query=None, gallery=None,
                 ...
                 cmc_topk=(1, 5, 10), cmc_flag=False, cam_overlap=False):
    ...
    if cam_overlap:
        mAP = mean_ap_nocam(distmat, query_ids, gallery_ids, query_cams, gallery_cams) 
    else:
    ...
    if cam_overlap:
        cmc_scores = {name: cmc_nocam(distmat, query_ids, gallery_ids,
...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, metric=None, cmc_flag=False, rerank=False, pre_features=None, cam_overlap=False):
        ...
        if (not rerank):
            results = evaluate_all(query_features, gallery_features, distmat, query=query, gallery=gallery, cmc_flag=cmc_flag, cam_overlap=cam_overlap)
        ...
        distmat_rerank = re_ranking(distmat.numpy(), distmat_qq.numpy(), distmat_gg.numpy())
        return evaluate_all(query_features, gallery_features, distmat_rerank, query=query, gallery=gallery, cmc_flag=cmc_flag, cam_overlap=cam_overlap), evaluate_all(query_features, gallery_features, distmat, query=query, gallery=gallery, cmc_flag=cmc_flag, cam_overlap=cam_overlap) 
