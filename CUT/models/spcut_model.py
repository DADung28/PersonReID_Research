...
class SPCUTModel(BaseModel):
    ...
    def optimize_parameters(self):
        ...
        self.loss_copilot.backward()
        self.optimizer_Copilot.step()
        
        # update G
        self.set_requires_grad([self.netD, self.netCopilot], False)
    ...
    def compute_copilot_loss(self):
        a_metric = self.netCopilot(self.real_A)
        a2b_metric = self.netCopilot(self.fake_B.detach())
        b_metric = self.netCopilot(self.real_B)
        
        loss_pos = self.contrastive(a_metric, a2b_metric, 0)
        loss_neg = self.contrastive(a2b_metric, b_metric, 1)
        # combine loss and calculate gradients
        self.loss_copilot = loss_pos + loss_neg
        return self.loss_copilot
    ...
    def compute_G_loss(self):
        ...
        loss_copilot = self.compute_copilot_loss()
        
        self.loss_G = self.loss_G_GAN + loss_NCE_both + loss_copilot * 2
