    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for iter, data in enumerate(dataloaders[phase]):
                ...
                elif opt.use_latrans or opt.use_latransv2 or opt.use_laswin or opt.use_laswinv2 or opt.use_caswin or opt.PCB:
                    score = 0.0
                    sm = nn.Softmax(dim=1)
                    loss = 0.0
                    if return_feature:
...
if opt.use_dense:
    ...
    model = LASwin(len(class_names), opt.droprate, opt.stride, circle = return_feature, linear_num=opt.linear_num, vertical=True)
