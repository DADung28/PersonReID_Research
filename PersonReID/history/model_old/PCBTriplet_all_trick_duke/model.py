...
class LASwin(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.circle:
            x_global = self.avgpool1(x_global.permute((0,2,1))) # Change shape from [batchsize, 49, 1024] -> [batchsize, 1024, 49] and do average pooling on 2 dimension 
            x_global = x_global.view(x_global.size(0), x_global.size(1))
            y_global = self.classifier(x_global)
        
        x_local = self.avgpool(x.permute(0,3,1,2)) # torch.Size([32, 1024, 7, 1])
...
class LASwinTest(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, circle=False, linear_num=512):
        ...
        self.model = model_ft
        self.circle = circle
        self.avgpool = nn.AdaptiveAvgPool2d((self.part, 1024))
        for i in range(self.part):
            name = 'classifier'+str(i)
            setattr(self, name, ClassBlock(1024, class_num, droprate, linear=linear_num, return_f = False))
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x) # torch.Size([32, 7, 1024])
...
class LATransformer(nn.Module):
    ...
    def forward(self,x):
        ...
        part = {}
        predict = {}
        # get 14 part feature batchsize*768*14*1
        for i in range(self.part):
...
class LATransformerV2(nn.Module):
    def __init__(self, class_num=751, droprate=0.5, stride=2, circle=False, ibn=False, linear_num=256):
        super().__init__()
        model = timm.create_model('vit_base_patch16_224', pretrained=True, num_classes=class_num)
        self.patch_embed = model.patch_embed
    ...
    def forward(self,x):
        ...
        # Average pool
        #x = self.avgpool(x[:, 1:])
        x = self.avgpool(x) # torch.Size([32, 14, 768])
        
        x = x.permute(0,2,1).unsqueeze(dim=-1) # torch.Size([32, 768, 14, 1])
        # Locally aware network
        part = {}
        predict = {}
        # get 14 part feature batchsize*768*14*1
...
# Define the ViT Model without clst oken
class ViTReIDRaw(nn.Module):
...
class PCB(nn.Module):
    def __init__(self, class_num, droprate=0.5, stride=2, circle=False, linear_num=1024):
        ...
        for i in range(self.part):
            name = 'classifier'+str(i)
            setattr(self, name, ClassBlock(2048, class_num, droprate=0.5, linear=256, relu=False, bnorm=True, return_f = self.circle))
            
        if self.circle:
            self.avgpool1 = nn.AdaptiveAvgPool1d(1)
            self.classifier = ClassBlock(2048, class_num, droprate=0.5, linear=linear_num, relu=False, bnorm=True, return_f = self.circle) 
...
class PCB_test(nn.Module):
    def __init__(self, class_num, droprate=0.5, stride=2, circle=False, linear_num=1024):
        ...
        for i in range(self.part):
            name = 'classifier'+str(i)
            setattr(self, name, ClassBlock(2048, class_num, droprate=0.5, linear=256, relu=False, bnorm=True, return_f = False))
