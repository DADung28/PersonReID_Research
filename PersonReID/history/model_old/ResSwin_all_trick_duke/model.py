...
class TripletSwin(nn.Module):
    ...
    def forward(self, x):
        ...
        x_2 = x.view(x.shape[0],-1,x.shape[-1]) # torch.Size([32, 196, 512])
        x_2 = self.avgpool(x_2.permute((0,2,1))).squeeze()
        x_2 = self.classifier(x_2)
         
        x = self.layers[3](x)
        x = self.norm(x)
        
        x_3 = x.view(x.shape[0],-1,x.shape[-1]) # torch.Size([32, 196, 512])
        x_3 = self.avgpool(x_3.permute((0,2,1))).squeeze() # torch.Size([32, 1024])
...
class O2LSwin(nn.Module):
    ...
    def forward(self, x):
        ...
        x_3 = self.relu(x_3) + self.relu(x_2)
        #print(x_3.shape)
        x_3 = self.classifier3(x_3)
        #print(x_3[0].shape, x_3[1].shape)
        #print(features.shape)
