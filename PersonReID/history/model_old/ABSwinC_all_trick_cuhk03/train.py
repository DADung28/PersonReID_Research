...
parser.add_argument('--use_laswin', action='store_true', help='use LASwin - An Local Attention ver of Swin Transformer' )
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for iter, data in enumerate(dataloaders[phase]):
                ...
                elif opt.use_latrans or opt.use_latransv2 or opt.use_laswin or opt.PCB:
                    score = 0.0
                    sm = nn.Softmax(dim=1)
                    loss = 0.0
                    if return_feature:
...
elif opt.use_latransv2:
    model = LATransformerV2(len(class_names), opt.droprate, circle = return_feature)
    print(len(class_names))
elif opt.use_vit:
    model = ViTReID(len(class_names), opt.droprate, circle = return_feature, linear_num=opt.linear_num)
elif opt.use_vitraw:
    model = ViTReIDRaw(len(class_names), opt.droprate, circle = return_feature, linear_num=opt.linear_num)
elif opt.use_laswin:
