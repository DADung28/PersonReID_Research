...
class O2LSwin(nn.Module):
    def __init__(self, class_num, droprate=0.5, stride=2, circle=False, linear_num=512, test=False):
        ...
        self.classifier0 = ClassBlock(512, class_num, droprate, linear=linear_num, return_f = self.circle)
        self.classifier1 = ClassBlock(1024, class_num, droprate, linear=linear_num, return_f = self.circle)
    ...
    def forward(self, x):
        ...
        x_2 = self.classifier0(x_2) 
        x = self.layers[3](x)
        x = self.norm(x)
        x_3 = self.avgpool(x.permute(0,3,1,2)).squeeze() # torch.Size([32, 1024])
        x_3 = self.classifier1(x_3) 
        #print(features.shape)
        if self.test:
            features = torch.cat((x_2[1],x_3[1]), dim=1)
            return features
        else: 
            return [x_2,x_3] 
