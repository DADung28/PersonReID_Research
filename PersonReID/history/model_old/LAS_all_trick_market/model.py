...
class LASwin(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.circle:
            x_global = self.avgpool1(x.permute((0,2,1))) # Change shape from [batchsize, 49, 1024] -> [batchsize, 1024, 49] and do average pooling on 2 dimension 
            x_global = x_global.view(x_global.size(0), x_global.size(1))
            y_global = self.classifier(x_global)
        
        x_local = self.avgpool(x) # torch.Size([32, 7, 1024])
        x_local = x_local.permute(0,2,1).unsqueeze(dim=-1) # torch.Size([32, 1024, 7, 1])
...
class LASwinTest(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, circle=False, linear_num=512):
        ...
        for i in range(self.part):
            name = 'classifier'+str(i)
            setattr(self, name, ClassBlock(1024, class_num, droprate, linear=linear_num, return_f = circle)) 
    def forward(self, x):
        x = self.model.forward_features(x)
        x = x.view(x.shape[0],-1,x.shape[-1]) # Change shape from [batchsize, 7, 7, 1024] -> [batchsize, 49, 1024]
