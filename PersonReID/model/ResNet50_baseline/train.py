...
from utils.part_erasing import PartErasing
...
# data
parser.add_argument('--data_dir',default='/home/jun/ReID_Dataset/duke_segmentation/dataloader',type=str, help='training dir path')
...
parser.add_argument('--use_lenet', action='store_true', help='use googlenet')
...
parser.add_argument('--use_latransv2', action='store_true', help='use LA_TransformerV2' ) # Not use global token
parser.add_argument('--use_laswin', action='store_true', help='use LASwin - An Local Attention ver of Swin Transformer Split Horizontal' )
parser.add_argument('--use_laswinv2', action='store_true', help='use LASwin - An Local Attention ver of Swin Transformer Split Vertical' )
parser.add_argument('--use_caswin', action='store_true', help='use LASwin - An Center Attention ver of Swin Transformer Split Vertical' )
parser.add_argument('--use_vit', action='store_true', help='use Vision Transformer (ViT) with cls token' )
parser.add_argument('--use_vitraw', action='store_true', help='use Vision Transformer (ViT) without cls token' )
...
parser.add_argument('--ABS', action='store_true', help='use swin transformer with all blocks loss' )
parser.add_argument('--O2LS', action='store_true', help='use swin transformer with feature from last two layer' )
parser.add_argument('--TripletSwin', action='store_true', help='use swin transformer with feature from last two layer' )
...
if opt.data_dir.find('cuhk03') != -1:
    opt.cuhk03 = True
print(f'Training on device: {opt.gpu_ids}')
# set gpu ids
if len(gpu_ids)==1:
...
#if opt.part_erasing>0:
#    transform_train_list = transform_train_list +  [PartErasing(model = parsing_model, probability = opt.part_erasing, mean=[0.0, 0.0, 0.0])] 
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for iter, data in enumerate(dataloaders[phase]):
                ...
                if return_feature and opt.use_latrans==False and opt.use_laswin==False and opt.PCB==False and opt.ABS==False and opt.O2LS==False and opt.TripletSwin==False: 
                    logits, ff = outputs
                ...
                #        loss += criterion(part[i+1], labels)
                elif opt.use_latrans or opt.use_latransv2 or opt.use_laswinv2 or opt.use_caswin or opt.PCB:
                    score = 0.0
                    sm = nn.Softmax(dim=1)
                    loss = 0.0
                    if return_feature:
                        logits_global, logits_local = outputs[0], outputs[1] # logits_global[0] (32, 751]) | logits_global[1] (32, 512) | logits_local[all] (32, 751)
                        score += sm(logits_global[0])
                        
                        ## Caculate loss on global feature
                        loss += criterion(logits_global[0], labels) 
                        if opt.circle: 
                            loss +=  criterion_circle(*convert_label_to_similarity(logits_global[1], labels))/now_batch_size
                        if opt.triplet:
                            loss += batch_hard_triplet_loss(labels, logits_global[1], margin = 6) #/now_batch_size
                        if opt.centroid:
                            loss += criterion_centroid(logits_global[1], labels)
                        ## Caculate loss on local feature
                        for logits in logits_local:
                            loss += criterion(logits[0], labels)
                            if opt.circle: 
                                loss +=  criterion_circle(*convert_label_to_similarity(logits[1], labels))/now_batch_size
                            if opt.triplet:
                                loss += batch_hard_triplet_loss(labels, logits[1], margin = 6) #/now_batch_size
                            if opt.centroid:
                                loss += criterion_centroid(logits[1], labels)
                    else:
                    ...
                    _, preds = torch.max(score.data, 1)
                elif opt.ABS  or opt.O2LS or opt.use_laswin :
                    score = 0.0
                    sm = nn.Softmax(dim=1)
                    loss = 0.0
                    if return_feature:
...
if opt.use_dense:
    ...
    model = GoogLeNet(len(class_names), opt.droprate, opt.stride, circle = return_feature, linear_num=opt.linear_num)
elif opt.use_NAS:
    model = ft_net_NAS(len(class_names), opt.droprate, linear_num=opt.linear_num)
elif opt.use_swin:
    model = Swin(len(class_names), opt.droprate, opt.stride, circle = return_feature)
elif opt.use_swinv2:
    ...
    model = LATransformer(len(class_names), opt.droprate, circle = return_feature)
elif opt.use_vit:
    model = ViTReID(len(class_names), opt.droprate, circle = return_feature, linear_num=opt.linear_num)
elif opt.use_laswin:
    model = LASwinV2(len(class_names), opt.droprate, opt.stride, circle = return_feature, linear_num=opt.linear_num, vertical=True)
elif opt.use_laswinv2:
    model = LASwin(len(class_names), opt.droprate, opt.stride, circle = return_feature, linear_num=opt.linear_num, vertical=False)

    #model = LASwin(len(class_names), opt.droprate, opt.stride, circle = return_feature, linear_num=opt.linear_num)
elif opt.use_caswin:
    model = CASwin(len(class_names), opt.droprate, opt.stride, circle = return_feature, linear_num=opt.linear_num)
elif opt.PCB:
    opt.linear_num = 1536
    model = PCB(len(class_names), circle=return_feature, linear_num=opt.linear_num)
elif opt.ABS:
    opt.linear_num = 512
    model = ABSwin(len(class_names), opt.droprate, opt.stride, circle = return_feature, linear_num=opt.linear_num)
elif opt.O2LS: 
    opt.linear_num = 1024
    model = O2LSwin(len(class_names), opt.droprate, opt.stride, circle = return_feature, linear_num=opt.linear_num)
elif opt.TripletSwin: 
    opt.linear_num = 512
    model = TripletSwin(len(class_names), opt.droprate, opt.stride, circle = return_feature, linear_num=opt.linear_num)
else:
...
if len(opt.gpu_ids)>1:
    ...
    if opt.PCB:
        ...
        ignored_params += (list(map(id, model.module.classifier0.parameters() ))
                     ...
                     +list(map(id, model.module.classifier6.parameters() ))
                     +list(map(id, model.module.classifier7.parameters() ))
                     +list(map(id, model.module.classifier8.parameters() ))
                     +list(map(id, model.module.classifier9.parameters() ))
                     +list(map(id, model.module.classifier10.parameters() ))
                     +list(map(id, model.module.classifier11.parameters() ))
                     +list(map(id, model.module.classifier12.parameters() ))
                     +list(map(id, model.module.classifier13.parameters() ))
                      )
        base_params = filter(lambda p: id(p) not in ignored_params, model.module.parameters())
        classifier_params = filter(lambda p: id(p) in ignored_params, model.module.parameters())
        optimizer_ft = optim_name([
