...
class LATransformer(nn.Module):
    ...
    def forward(self,x):
        ...
        for i in range(self.part):
            out = torch.mul(cls_token_out, self.lmbd)
            x[:,i,:] = torch.div(torch.add(out.squeeze(),x[:,i,:]), 1+self.lmbd)
...
class GoogLeNet(nn.Module):
    ...
    def __init__(self, class_num=751, droprate=0.5, stride=2, circle=False, ibn=False, linear_num=512):
        ...
        self.fc = model.fc
        self.circle = circle
        self.classifier = ClassBlock(832, class_num, droprate, linear=linear_num, return_f = circle)
