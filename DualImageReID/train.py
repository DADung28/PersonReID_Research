...
parser.add_argument('--hrnet', action='store_true', help='use ResNet50 (ViT) with cls token' )
parser.add_argument('--pivit', action='store_true', help='use Vision Transformer (ViT) with cls token' )
parser.add_argument('--pivit_3loss', action='store_true', help='use Vision Transformer (ViT) with cls token' )
parser.add_argument('--piswin', action='store_true', help='use Vision Transformer (ViT) with cls token' )
parser.add_argument('--piswin_3loss', action='store_true', help='use Vision Transformer (ViT) with cls token' )
parser.add_argument('--piresnet', action='store_true', help='use Vision Transformer (ViT) with cls token' )
parser.add_argument('--pihrnet', action='store_true', help='use Vision Transformer (ViT) with cls token' )
parser.add_argument('--piresnet_3loss', action='store_true', help='use Vision Transformer (ViT) with cls token' )
parser.add_argument('--spresnet', action='store_true', help='use ResNet50 (ViT) with cls token' )
parser.add_argument('--spswin', action='store_true', help='use ResNet50 (ViT) with cls token' )
parser.add_argument('--sphrnet', action='store_true', help='use ResNet50 (ViT) with cls token' )
parser.add_argument('--spvit', action='store_true', help='use ResNet50 (ViT) with cls token' )
parser.add_argument('--spresnet_v2', action='store_true', help='use ResNet50 (ViT) with cls token' )
parser.add_argument('--singleresnet', action='store_true', help='use ResNet50 (ViT) with cls token' )
parser.add_argument('--singlehrnet', action='store_true', help='use ResNet50 (ViT) with cls token' )
parser.add_argument('--singlevit', action='store_true', help='use ResNet50 (ViT) with cls token' )
parser.add_argument('--singleswin', action='store_true', help='use ResNet50 (ViT) with cls token' )
...
if opt.vit:
    #opt.linear_num = 512
    model = ViT(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num, size=h)
elif opt.swin:
    #opt.linear_num = 512
    ...
    #opt.linear_num = 1024
    model = HRNet(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num)
elif opt.pivit:
    #opt.linear_num = 512
    model = PIViT(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num, size=h)
elif opt.singlevit:
    #opt.linear_num = 512
    model = SingleViT(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num, size=h)
elif opt.pivit_3loss:
    #opt.linear_num = 512
    model = PIViT_3loss(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num, size=h)
elif opt.piswin:
    #opt.linear_num = 512
    model = PISwin(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num, size=h)
elif opt.singleswin:
    #opt.linear_num = 512
    model = SingleSwin(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num, size=h)
elif opt.piswin_3loss:
    #opt.linear_num = 512
    model = PISwin_3loss(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num, size=h)
elif opt.piresnet:
    #opt.linear_num = 1024
    model = PIResNet50(len(class_names), opt.droprate, opt.stride, test=False, linear_num=opt.linear_num)
elif opt.singleresnet:
    #opt.linear_num = 1024
    model = SingleResNet50(len(class_names), opt.droprate, opt.stride, test=False, linear_num=opt.linear_num)
elif opt.pihrnet:
    #opt.linear_num = 1024
    model = PIHRNet(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num)
elif opt.singlehrnet:
    #opt.linear_num = 1024
    model = SingleHRNet(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num)
elif opt.piresnet_3loss:
    #opt.linear_num = 1024
    model = PIResNet50_3loss(len(class_names), opt.droprate, opt.stride, test=False, linear_num=opt.linear_num)
elif opt.spresnet:
    #opt.linear_num = 1024
    model = SPResNet50(len(class_names), opt.droprate, opt.stride, test=False, linear_num=opt.linear_num)
elif opt.spswin:
    #opt.linear_num = 1024
    model = SPSwin(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num, size=h)
elif opt.spvit:
    #opt.linear_num = 1024
    model = SPViT(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num, size=h)
elif opt.sphrnet:
    #opt.linear_num = 1024
    model = SPHRNet(len(class_names), opt.droprate, test=False, linear_num=opt.linear_num)
elif opt.spresnet_v2:
    #opt.linear_num = 1024
    model = SPResNet50_v2(len(class_names), opt.droprate, opt.stride, test=False, linear_num=opt.linear_num)
else:    
    model = ResNet50(len(class_names), opt.droprate, opt.stride, test=False, linear_num=opt.linear_num)
...
if opt.piresnet_3loss or opt.piswin_3loss or opt.pivit_3loss or opt.spresnet or opt.spresnet_v2 or opt.spswin or opt.sphrnet or opt.spvit:
